...
def to_images(images: np.array, target_dir, file_name='image'):
    shape = images.shape
    x = shape[0]
    ...
    for index in range(x):
        image = images[index, ...]
        imageio.imwrite(target_dir + os.sep + file_name + str(index + 1) + '.jpg', image, 'jpg')


def to_images_from(root: str, new_dir: str, dirs=(), do_resample=False):
    if not os.path.exists(new_dir):
        os.makedirs(new_dir)

    for dir_name in os.listdir(root):

        pat_path = root + os.sep + dir_name
        pat_path_1 = new_dir + os.sep + dir_name

        for sub_dir in dirs:
            path = pat_path + os.sep + sub_dir
            target = pat_path_1 + os.sep + sub_dir
            ...
            slices_voxel = get_pixels(slices)

            if do_resample:
                slices_voxel, _ = resample(slices_voxel, scan=slices)

            to_images(slices_voxel, target)
...
def load_sigle_volfile(datafile, np_var='vol'):
    """
